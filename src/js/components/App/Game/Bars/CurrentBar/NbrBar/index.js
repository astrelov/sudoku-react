...
export const NbrBar = ({nbrsAmount, selectedNbr, handlers}) => {
  ...
  for (let nbr = 1; nbr <= 9; nbr++) {
    const nbrAmountToFill = (9 - nbrsAmount[nbr]) < 0 ? '+' + (nbrsAmount[nbr]
        - 9) : (9 - nbrsAmount[nbr]) || '';
    ...
    buttons.push(
        <button
            ...
            onClick={e => handleSelectNbr(nbr,
                e)} // TODO remove this event pass
            key={nbr.toString()}
        >
          <div className={[style.nbr, text].join(' ')}>{nbr}</div>
          <div className={[style.nbrAmount, text].join(
              ' ')}>{nbrAmountToFill}</div>
        </button>
    );
  }
