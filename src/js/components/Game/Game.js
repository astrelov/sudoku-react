...
import { ControlBar } from './GameBars/ControlBar/ControlBar'
import { ChangesList } from '../../changes_list'
import { Changes } from '../../changes'
import { CurrentBar } from './GameBars/CurrentBar/CurrentBar'
...
export class Game extends React.Component {
  constructor(props) {
    ...
    this.initialField = this.cloneField(field)
    this.changesList = new ChangesList()
    this.setGameClock()

    this.state = {
      field,
      nbrsAmount: this.countNbrs(field),
      selectedNbr: 0,
      gameTime: 0,
      isWin: false,
    ...
    this.handlers = {
      ...
      handleSelectNbr: (nbr, { target }) => {
        const nbrIsSelected = target.classList.contains('selected')
      ...
      handleInput: (x, y) => {
        ...
        if (updateValueAt.call(this, x, y)) {
          this.setState({ field: this.state.field, nbrsAmount: this.state.nbrsAmount })
      ...
      handleRS: () => {
        this.setState({
          field: this.cloneField(this.initialField),
          nbrsAmount: this.countNbrs(this.initialField),
          confirming: false,
          isWin: false,
          currentBarName: 'nbr',
        })
  ...
  updateValueAt(x, y, value) {
    if (!Array.isArray(this.state.field[x][y])) {
      this.state.nbrsAmount[this.state.field[x][y]]--
    }
    this.state.field[x][y] = value
    if (!Array.isArray(this.state.field[x][y])) {
      this.state.nbrsAmount[this.state.field[x][y]]++
    }
  }
